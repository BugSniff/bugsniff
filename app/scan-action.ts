...
export async function requestScan(formData: FormData) {
  // Where to come back to when something goes wrong. Two forms start a scan —
  // the landing and the empty panel — and each has to keep the person where
  // they were. Matched against a list rather than trusted: it goes into a
  // redirect, and a path taken from the form is a path a stranger can write.
  const voltar = FROM.includes(String(formData.get("voltar")))
    ? String(formData.get("voltar"))
    : "/";
